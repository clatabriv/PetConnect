...
export class PanelAdminComponent implements OnInit {
  ...
  get animalesFiltrados(): Animal[] {
    if (!this.filtroRefugioId) return this.animales;
  ...
  guardarUsuario() {
    ...
    if (emailRepetido) this.errorUsuario['email'] = 'Ya existe un usuario con este correo';
    ...
    if (telefonoRepetido) this.errorUsuario['telefono'] = 'Ya existe un usuario con este teléfono';

    if (Object.keys(this.errorUsuario).length > 0) return;
    ...
    if (this.editandoUsuarioId) {
      ...
      const crear$ =
        usuarioData.rol === 'ADMIN'
          ? this.api.crearUsuarioAdmin(usuarioData)
  ...
  cargarAnimales() {
    this.api.getAnimales().subscribe((data) => (this.animales = data));
  }

  guardarAnimal() {
    this.errorAnimal = {};
    if (!this.formAnimal.nombre?.trim()) this.errorAnimal['nombre'] = 'El nombre es obligatorio';
    if (!this.formAnimal.especie?.trim()) this.errorAnimal['especie'] = 'La especie es obligatoria';
    if (!this.formAnimal.genero?.trim()) this.errorAnimal['genero'] = 'El género es obligatorio';
    if (!this.formAnimal.refugioId) this.errorAnimal['refugioId'] = 'El refugio es obligatorio';
  ...
  cargarRefugiosPendientes() {
    this.api.getRefugiosPendientes().subscribe((data) => (this.refugiosPendientes = data));
  }
