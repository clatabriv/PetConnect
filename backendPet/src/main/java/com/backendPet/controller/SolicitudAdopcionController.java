...
public class SolicitudAdopcionController {
    ...
    // Comprobar si ya existe solicitud pendiente entre adoptante y animal
    @PreAuthorize("hasRole('ADOPTANTE') or hasRole('ADMIN')")
    @GetMapping("/adoptantes/{adoptanteId}/animales/{animalId}/solicitudes/existe")
    public Map<String, Boolean> existeSolicitud(
            @PathVariable Long adoptanteId,
            @PathVariable Long animalId) {
        boolean existe = solicitudService.existeSolicitudPendiente(adoptanteId, animalId);
        return Map.of("yaSolicitado", existe);
    }
